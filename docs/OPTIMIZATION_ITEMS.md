...
本文记录 Qt/C++ 客户端审阅后的历史优化项，按优先级排列。标记为“已修复”的条目已有实现和 QtTest 覆盖。当前版本已把上游连接参数迁移到 SQLite“上游配置”，旧扁平 `upstream_*` 和协议拆分代理字段只用于升级迁移；下文涉及这些字段的案例描述的是旧版行为，不是现行配置方式。
...
- 该问题最初通过保留拆分字段及 split-only round-trip QtTest 修复。
- 当前版本进一步采用 SQLite 上游配置，每条配置只保存一个 `upstream_proxy`，空值表示直连。
- 旧通用/HTTP/HTTPS/SOCKS 字段只在一次性迁移时按 Base URL 和旧优先级折算为一个实际代理；正常保存不再写回这些字段。
...
1. 已完成并被上游配置取代：修复协议拆分代理字段配置语义；当前只在旧配置迁移中读取拆分字段。
